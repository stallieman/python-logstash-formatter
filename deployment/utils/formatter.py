...
def check_pipeline_file(file_path):
    """
    Lees een Logstash pipeline configuratie bestand en formatteer deze.
    
    Args:
        file_path (str): Pad naar het configuratie bestand
        
    Returns:
        tuple: (formatted_content, errors, fixes_applied)
    """
    try:
        ...
        return None, [f"Fout bij lezen van bestand: {str(e)}"], []


def check_pipeline_text(pipeline_text):
    """
    Formatteer Logstash pipeline configuratie tekst.
    
    Args:
        pipeline_text (str): De pipeline configuratie als tekst
    ...
    try:
        ...
        return None, [f"Fout bij verwerken van tekst: {str(e)}"], []
